...
(() => { // IIFE to prevent variable collisions with other modules
    ...
    // decision about the order, not about the basket. Everything else about
    // this checkout attempt is not: on success this leaves nothing behind on
    // checkout.html itself — draft, idempotency key, the resumed handshake —
    // and sends the customer back to the store rather than repainting an
    // empty form in place, so a stale idempotency key from this attempt can
    ...
    async function cancelPendingOrder(button) {
        ...
        // No retained information: this attempt is over, and clearDraft()
        // covers the idempotency key as well as the typed contact/address —
        // a stronger reset than rotateIdempotencyKey() alone, appropriate
        // here because nothing on this page is about to be repainted from it.
        rememberPending(null);
        clearDraft();
        // Back to the store, not a repainted checkout form — the basket is
        // untouched, so it is exactly where the customer left it.
