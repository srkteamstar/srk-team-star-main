...
(() => {
    ...
    function dot(product, index) {
        ...
        return '<button type="button" role="tab" data-hero-dot="' + index + '"' +
            ' aria-label="Show ' + escapeHtml(product.name) + '"' +
            ' aria-selected="' + (active ? 'true' : 'false') + '"' +
            ' class="h-1.5 rounded-full transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#d4af37] ' +
            (active ? 'w-7 bg-[#d4af37]' : 'w-2.5 bg-white/35 hover:bg-white/60') + '"></button>';
    }
    ...
    function render(section, products) {
        ...
        const dots = loops
            ? '<div class="absolute bottom-[31px] md:bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-2 z-20" role="tablist" aria-label="Choose featured product">' +
        ...
        function syncDots() {
            ...
            dotEls.forEach((el, i) => {
                ...
                el.classList.toggle('w-7', active);
                el.classList.toggle('bg-[#d4af37]', active);
                el.classList.toggle('w-2.5', !active);
                el.classList.toggle('bg-white/35', !active);
                el.classList.toggle('hover:bg-white/60', !active);
            });
        }
