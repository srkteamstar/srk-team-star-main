// ==========================================
// 1. HORIZONTAL CAROUSEL MODULE
// ==========================================
const carouselData = [
    { title: 'Machinery', image: '/assets/machine-card-desktop.png', heading: 'Engineered for Excellence', description: 'Industrial-grade machinery delivering precision engineering, dependable performance, and the productivity modern workshops demand.', ctaText: 'Explore Collection', ctaLink: '/products' },
    { title: 'Frame Mouldings', image: '/assets/frame-moulding-card-desktop.png', heading: 'The Art Around the Art', description: 'Beautifully crafted mouldings that transform every photograph and artwork into a timeless display.', ctaText: 'View Collection', ctaLink: '/products' },
    { title: 'Frame Hardware', image: '/assets/hardware-card-desktop.png', heading: 'The Strength Behind Every Frame', description: 'Premium hardware components designed to provide secure assembly, clean finishes, and lasting durability.', ctaText: 'Shop Collection', ctaLink: '/products' },
];

class HorizontalCarousel {
    constructor(containerId, items) {
        this.container = document.getElementById(containerId);
        this.leftBtn = document.getElementById('carousel-left-btn');
        this.rightBtn = document.getElementById('carousel-right-btn');
        this.categoryTitle = document.getElementById('category-title');
        this.categoryHeading = document.getElementById('category-heading');
        this.categoryDescription = document.getElementById('category-description');
        this.categoryCta = document.getElementById('category-cta');
        this.ctaActionBar = document.querySelector('.cta-action-bar');
    ...
    render() {
        ...
        this.items.forEach((item, index) => {
            ...
            card.innerHTML = `<img src="${item.image}" alt="${item.title}" class="w-full h-full object-cover pointer-events-none" />`;
    ...
    updateTextContent() {
        ...
        if (this.categoryCta) {
            ...
            this.categoryCta.href = currentData.ctaLink || '/products';
        }
...
document.addEventListener('DOMContentLoaded', () => {
    if (document.getElementById('horizontal-carousel')) {
        new HorizontalCarousel('horizontal-carousel', carouselData);
    }
});