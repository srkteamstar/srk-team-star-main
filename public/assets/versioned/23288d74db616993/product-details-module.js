...
(() => {
    ...
    function ledeOf(row) {
        ...
        return featured || plain || String(row.name || 'This product') + ' is listed in our ' + String(row.category_name || 'frame-making') + ' range. Contact us to confirm specifications and compatibility.';
    }
    ...
    async function submit(thenOpenCart) {
        ...
        try {
            ...
            // cart-module.js's flushPendingWrite() now surfaces a failed
            // save instead of swallowing it. This overlay is already closed,
            // so open the cart drawer instead — it shows the same cart
            // error banner and retry path a failure on its own checkout
            // link would.
            if (typeof window.storeCart.open === 'function') window.storeCart.open();
        }
    }
    ...
    function attach() {
        ...
        document.addEventListener('click', (event) => {
            ...
            // Ordinary activation keeps the existing overlay. Modified clicks
            // and new tabs retain the real, server-rendered product destination.
        ...
        document.addEventListener('keydown', (event) => {
            ...
            if (event.key === 'Enter') return; // native link click handles Enter
