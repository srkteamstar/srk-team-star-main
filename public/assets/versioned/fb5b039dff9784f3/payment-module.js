...
(function () {
    ...
    // Neither onload nor onerror is guaranteed to fire for every kind of
    // stall — a proxy that accepts the connection and then never answers, for
    // one — and without a bound the caller's button stayed on "Opening
    // payment…" forever with nothing to recover from but a reload.
    const SCRIPT_LOAD_TIMEOUT_MS = 15000;

    // The confirmation request has to answer before this page can tell the
    // customer anything. A hung request otherwise held `settled` unset
    // indefinitely, silently ignoring a dismiss and leaving no bounded
    // fallback to run.
    ...
    async function loadCheckoutScript(targetWindow) {
        ...
        const scriptPromise = new Promise((resolve, reject) => {
            ...
            // The bound. onload/onerror are not guaranteed for every kind of
            // stall (a proxy that accepts the connection and never answers),
            // and without this the caller's button stayed disabled forever.
            const timer = setTimeout(() => {
                if (settled) return;
                settled = true;
                scriptPromises.delete(host);
                reject(new Error('Could not reach the payment provider (timed out).'));
    ...
    async function confirmWithServer(orderId, response) {
        // AbortController, not just a race: a stray fetch left running past
        // its timeout would still be able to resolve the SAME `settled` flag
        // pay()'s handler guards with, arriving after the caller has already
        // moved on. Aborting it stops that from ever being possible.
