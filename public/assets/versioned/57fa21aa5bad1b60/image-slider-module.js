...
class HorizontalCarousel {
    ...
    // Built via createElement/property assignment rather than innerHTML
    // string interpolation. `item.title` and `item.image` come from
    // `upcoming_projects` (database-sourced, via fetchUpcomingProjectsData
    // above) — an admin-entered title containing a `"` used to be able to
    // break out of the `alt="..."` attribute and inject markup into the
    // page for every visitor. Property assignment (img.src, img.alt) can
    // never be parsed as markup, so this is safe regardless of content.
    render() {
