...
(() => {
    ...
    function mountGallery(hero, entries) {
        ...
        function prepareNext() {
            if (reduced || !ready[0] || slides.length < 2 || document.hidden || !inView || nextLoad !== null) return;
            nextLoad = window.setTimeout(() => {
                nextLoad = null;
                if (!document.hidden && inView) requestImage((current + 1) % slides.length);
            }, 300);
        }
        ...
        function start() {
            if (reduced || !ready[0] || slides.length < 2 || document.hidden || !inView || timer) return;
