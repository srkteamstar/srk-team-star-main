...
(() => { // IIFE to prevent variable collisions with other modules
    ...
    // resolves.
    // THE FAILURE IS NO LONGER SWALLOWED HERE (F02). This used to return
    // `lastSave.catch(() => {})`, so a caller that awaited it — the checkout
    // link and Buy Now below — always saw a resolved promise and always
    // navigated, even when the write it was supposedly waiting for had
    // actually failed. The banner (cartSaveErrorMessage / showCartSaveError)
    // already fires regardless, via enqueueSave()'s own
    // `lastSave.catch(showCartSaveError)`; what was missing was giving a
    // caller that specifically needs to know — not just display — a way to
    // find out and stay put instead of moving on to a page built on a basket
        ...
        node.addEventListener('click', (event) => {
            ...
            if (target.id === 'cart-checkout') {
                ...
                flushPendingWrite().then(() => {
                    ...
                    // F02 — do not navigate over a save that just failed.
                    // showCartSaveError already ran (enqueueSave's own
                    // lastSave.catch) and paint() already redrew this same
                    // open drawer with the banner — nothing further to do
                    // here except NOT leave the page.
                });
                return;
            }
            ...
            add(productId).then(async added => {
                ...
                if (buyButton) {
                    try {
                        ...
                        // F02 — the save just failed, so this stays on the
                        // page instead of continuing to a checkout built on
                        // a basket the server never actually saved. Opening
                        // the drawer is what makes showCartSaveError's
                        // banner (already fired by enqueueSave) visible —
                        // there was no open drawer for paint() to redraw.
                        open();
                    }
                }
    ...
    // Nothing here can react to a failure at this point — the page is
    // already going away — so these two call sites are the ones place that
    // deliberately still swallow flushPendingWrite()'s rejection, purely to
    // avoid an unhandled-rejection warning for a promise nothing is waiting
    // on. showCartSaveError still ran; there is just no drawer left open to
    // paint it into.
    window.addEventListener('pagehide', () => { flushPendingWrite().catch(() => {}); });
    document.addEventListener('visibilitychange', () => {
        if (document.visibilityState === 'hidden') flushPendingWrite().catch(() => {});
    });
