...
(() => { // IIFE to prevent variable collisions with other modules
    ...
    // but the customer has NOT said they are done — returnToCheckoutAfterFailure()'s
    // own comment is explicit that a cancel lands back on a filled form, not
    // an empty one. Without this, the retired order's key stayed in `draft` and
    ...
    async function resumeIfPending() {
        ...
        // that startPayment() actually works from. On the offline path below
        // — server unreachable, saved copy used — the awaiting screen would
        // otherwise paint with a Pay-now button that reported "online payment
        // is unavailable" the moment it was pressed, which is the failure
    ...
    // Self-serve cancellation used to POST straight to
    ...
    // through support instead, so the button now only opens a notice —
    // nothing is sent to the server, and pendingPayment/the order are left
    // exactly as they are.
    //
    // requestOrderCancellation() itself is untouched and still runs
    // automatically — from returnToCheckoutAfterFailure() after a failed
    // Razorpay attempt, and from the payment-method-switch path above — and
    // neither of those is the customer pressing this button, so neither
    // shows a popup.
    function showCancelSupportNotice() {
        const reference = pendingPayment && pendingPayment.reference;
        ...
        // not reachable from a single click on the awaiting-payment screen.
    }
    ...
    root.addEventListener('click', (event) => {
        ...
        if (target.id === 'checkout-cancel-order') return showCancelSupportNotice();
