...
(function () {
    ...
    function harden(el) {
        ...
        // Personal-information fields expose their purpose to assistive tools
        // and browser autofill. Search/filter/quantity fields keep the existing
        // no-suggestions policy, and credential exceptions remain unchanged.
        var purposes = {
            'form-name': 'name', 'form-company': 'organization', 'form-email': 'email', 'form-phone': 'tel',
            'checkout-name': 'name', 'checkout-phone': 'tel', 'checkout-email': 'email', 'checkout-company': 'organization',
            'checkout-address': 'street-address', 'checkout-city': 'address-level2', 'checkout-state': 'address-level1', 'checkout-postal': 'postal-code', 'checkout-country': 'country-name',
            'quote-business-name': 'organization', 'quote-contact-name': 'name', 'quote-email': 'email', 'quote-phone': 'tel', 'quote-business-address': 'street-address',
            'account-name': 'name', 'account-phone': 'tel', 'account-company': 'organization', 'account-address': 'street-address',
            'account-city': 'address-level2', 'account-state': 'address-level1', 'account-postal': 'postal-code', 'account-country': 'country-name'
        };
        if (purposes[el.id]) {
            el.setAttribute('autocomplete', purposes[el.id]);
            return;
        }
