document.addEventListener("DOMContentLoaded", () => {
    ...
    const policyTemplates = {
        ...
        return: `
            ...
            <section class="max-w-4xl text-[#1F271B] text-base leading-relaxed">
                ...
                <h2 class="text-xl font-bold mt-8 mb-3 text-[#12170F]">Cancellations and refund eligibility</h2><p class="mb-6">A refund is issued only for an order that has already been paid and is then cancelled — either cancelled by us (for example, where we are unable to fulfil it), or cancelled at your request after you contact our team using the details below. There is no automated, instant self-service cancellation or refund on the site; every cancellation is reviewed and actioned by a member of our team. This does not limit the separate right described above to a repair, replacement, return or refund for goods that are damaged, defective, deficient, spurious or materially different from their description, which applies regardless of whether an order has been cancelled.</p>
    ...
    function updatePageMetadata(route) {
        if (!route) return;
        document.title = /SRK Team Star/i.test(route.title) ? route.title : route.title + ' | SRK Team Star';
        const description = 'Read the ' + route.title.replace(/\s*\|.*$/, '') + ' for SRK Team Star products, purchases and customer support.';
        ...
        if (canonical) {
            ...
            url.pathname = route.path; url.search = ''; url.hash = '';
    ...
    window.addEventListener('popstate', () => {
        ...
        updatePageMetadata(policyRoutes[key]);
    });
