...
(() => { // IIFE to prevent variable collisions with other modules
    ...
    function itemsHTML(order) {
        ...
        return [
            ...
            '</div>',
            // VRL Logistics is the one courier every order ships with, so
            // this is a fixed label rather than something read off the
            // order — there is nothing per-order to name. Shown only once
            // there is a tracking value: before dispatch no courier has
            // actually been engaged yet, so naming one earlier would be
            // stating something that has not happened as if it had.
            order.tracking
                ? '    <div class="text-[#1f271b]/50 font-bold uppercase text-[10px] tracking-wider pt-0.5">Courier</div>' +
                  '\n    <div class="text-[#12170f] font-bold text-xs">VRL Logistics</div>'
                : '',
