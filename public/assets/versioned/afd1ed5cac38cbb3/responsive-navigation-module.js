(function () {
    ...
    function offCanvasShell(shell, sidebar, main, kind) {
        ...
        const close = () => {
            ...
            const destination = document.activeElement?.closest('[role="dialog"],[role="alertdialog"]');
            const destinationHasFocus = destination && destination !== sidebar;
            releaseSidebar();
            sidebarOpen = false;
            ...
            unlockPage();
            button.setAttribute('aria-expanded', 'false');
            if (!destinationHasFocus) button.focus();
        };
        ...
        sidebar.addEventListener('click', event => {
            // Release during the click. A document-level capture handler can
            // already have opened a dialog, so close also preserves any focus
