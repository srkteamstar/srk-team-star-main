-- A lost checkout response followed by a retry used to create a second
-- order for the same basket: nothing recognised the retry as the SAME
-- attempt rather than a new one. checkout.controller.js now checks for an
-- existing order carrying the browser's own key before writing a new one,
-- and this column plus its unique index are what make that check meaningful
-- — including collapsing two near-simultaneous identical requests to one
-- row instead of two, via the index rather than a race in application code.
...
-- create_store_order (025, redefined by 030/031) needs the one new field
-- threaded through to the INSERT — CREATE OR REPLACE restates the whole
-- function body, so this is 031's body plus idempotency_key, not a diff.
...
comment on function public.create_store_order(bigint,jsonb,jsonb,jsonb,jsonb) is
    'Server-only atomic checkout write for either an account owner or a token-protected guest. Carries the browser''s idempotency key so a lost-response retry lands on the same order.';
